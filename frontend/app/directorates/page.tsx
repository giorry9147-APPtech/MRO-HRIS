...
export default function DirectoratesPage() {
	const { user, loading, forbidden } = useAuthGuard("directorates.view");
	const [items, setItems] = useState<Array<{ id: number; name: string; code: string | null; status: string }>>([]);
	...
	const canCreate = hasPermission(user, "directorates.create");
	const canDelete = hasPermission(user, "directorates.delete");
	...
	async function handleCreate(event: FormEvent<HTMLFormElement>) {
		...
		try {
			if (editingId) {
				const updated = await apiFetch<{ data: { id: number; name: string; code: string | null; status: string } }>(`/directorates/${editingId}`, {
					method: "PATCH",
					body: JSON.stringify({ name, code: code || null }),
				});
				setItems((prev) => prev.map((item) => item.id === editingId ? updated.data : item));
			} else {
				const created = await apiFetch<{ data: { id: number; name: string; code: string | null; status: string } }>("/directorates", {
					method: "POST",
	...
	async function handleDelete(id: number) {
		await apiFetch<{ message: string }>(`/directorates/${id}`, { method: "DELETE" });
